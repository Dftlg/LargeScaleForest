...
void VolumetricMesh::checkPoint4Element(const char *vfilename,const char* voutputfile)
{
	std::ifstream vertexFile(vfilename);
	if (!vertexFile.is_open())
	{
		printf("Error: could not open vertex file %s.\n", vfilename);
		throw 1;
	}
	std::string tempString;
	vector<int> vertexNumbers;
	while (getline(vertexFile, tempString))
	{
		char * inputString = (char*)tempString.c_str();
		const char * split = ",";
		char *p = strtok(inputString, split);
		int Index;
		while (p != NULL)
		{
			// char * -> int
			sscanf(p, "%d", &Index);
			vertexNumbers.push_back(Index);
			p = strtok(NULL, split);
		}
	}
	vertexFile.close();
	set<int> elementNumbers;
	for (auto vertexNumber : vertexNumbers)
	{
		for (auto i = 0; i < numElements; i++)
		{
			for (auto j = 0; j < numElementVertices; j++)
			{
				if (elements[i][j] == vertexNumber)
				{
					/*int a = elements[i][0];
					int b = elements[i][1];
					int c = elements[i][2];
					...
					int g = elements[i][6];
					int h = elements[i][7];*/
					elementNumbers.insert(i+1);
				}			
			}
		}
	}
	std::ofstream elementFile(voutputfile);
	if (!elementFile.is_open())
	{
		printf("Error: could not open element file %s.\n", voutputfile);
		throw 1;
	}
	for (set<int>::iterator it = elementNumbers.begin(); it != elementNumbers.end(); it++)
	{
		elementFile << *it << ",";
	}
	elementFile.close();
