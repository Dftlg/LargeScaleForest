...
{    
    ...
    vec3 V = normalize(camPos - v3f_WorldPos);
	 ...
	 for(int i = 0; i < 4; ++i) 
    {
        ...
        vec3 L = normalize(lightPositions[i] - v3f_WorldPos);
        vec3 H = normalize(V + L);
        float distance = length(lightPositions[i] - v3f_WorldPos);
